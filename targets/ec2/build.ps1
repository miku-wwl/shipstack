...
#
# 第二阶段：触发 LocalStack CodePipeline，并等待 CodeBuild 通过 SSM 部署到 EC2
...
switch ($Stage) {
    "Upload" {
        ...
        $ArchivePath = Join-Path ([System.IO.Path]::GetTempPath()) "shipstack-ec2-source-$([guid]::NewGuid()).zip"
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0 -or $TrackedTargetFiles.Count -eq 0) {
                throw "当前 Git HEAD 不包含 $TargetRelativePath。请先提交 EC2 目标，再上传源码。"
            }
        ...
        do {
            ...
            if ($null -ne $Execution) {
                ...
                if ($Status -eq "Succeeded") {
                    Write-Host "CodePipeline 成功，CodeBuild 已通过 SSM 完成 EC2 部署和运行校验。"
                    exit 0
                }
