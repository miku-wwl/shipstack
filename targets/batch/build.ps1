...
#
# 第二阶段：触发 LocalStack CodePipeline，并等待 Batch Job 完成与结果校验
...
switch ($Stage) {
    "Upload" {
        ...
        $ArchivePath = Join-Path ([System.IO.Path]::GetTempPath()) "shipstack-batch-source-$([guid]::NewGuid()).zip"
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0 -or $TrackedTargetFiles.Count -eq 0) {
                throw "当前 Git HEAD 不包含 $TargetRelativePath。请先提交 Batch 目标，再上传源码。"
            }
        ...
        do {
            ...
            if ($null -ne $Execution) {
                ...
                if ($Status -eq "Succeeded") {
                    Write-Host "CodePipeline 执行成功，已完成 Batch 镜像构建、Job 提交和 release 输出校验。"
                    exit 0
                }
