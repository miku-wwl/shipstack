...
#
# 第二阶段：触发 LocalStack CodePipeline，并等待部署完成
...
switch ($Stage) {
    "Upload" {
        ...
        $ArchivePath = Join-Path ([System.IO.Path]::GetTempPath()) "shipstack-source-$([guid]::NewGuid()).zip"

        try {
            Write-Host "从当前 Git HEAD 创建源码归档：$ArchivePath"
            & git -C $TargetDirectory archive --format=zip "--output=$ArchivePath" HEAD
            if ($LASTEXITCODE -ne 0) {
        ...
        do {
            ...
            if ($null -ne $Execution) {
                ...
                if ($Status -eq "Succeeded") {
                    ...
                    Write-Host "CodePipeline 执行成功，已完成从 S3 下载源码并执行后续部署。"
                    exit 0
                }
