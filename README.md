...
当前 ECS、EC2、EKS、Elastic Beanstalk 和 Lambda 目标主要面向 **LocalStack Ultimate**，分别学习容器镜像、EC2 主机 JAR、Kubernetes 镜像、托管应用平台版本和 Lambda 部署包五种交付模型。目标都使用传统的 CodePipeline V1 -> CodeBuild 学习路径；EC2 目标在 CodeBuild 中显式使用 SSM 完成主机部署，EKS 目标在 CodeBuild 中显式使用 kubectl 完成 Kubernetes 部署，Beanstalk 目标在 CodeBuild 中显式调用 Elastic Beanstalk API 完成版本发布与环境更新。
...
建议先阅读 [`targets/ecs/README.md`](targets/ecs/README.md) 了解容器交付，再阅读 [`targets/eks/README.md`](targets/eks/README.md) 对比 Kubernetes 的 Deployment、Pod 和 Service，然后阅读 [`targets/ec2/README.md`](targets/ec2/README.md) 观察主机与进程细节，接着阅读 [`targets/beanstalk/README.md`](targets/beanstalk/README.md) 理解 PaaS 的应用版本与环境，最后阅读 [`targets/lambda/README.md`](targets/lambda/README.md) 比较函数部署包交付。
